...
public class NetHttp {
    ...
    private final String TAG = "NetHttp";
    private static NetHttp netHttp;

    public static NetHttp getNet() {
        if (null == netHttp) {
    ...
    private void req(String url, final NetCallBack<String> netCallBack) {
        LogUtils.d(TAG, "-->>微聊:" + url);
        if (!hasNet()) {
            if (netCallBack != null) {
                netCallBack.onFail();
                LogUtils.d(TAG, "-->>微聊,没有网络");
            }
            return;
        }
                ...
                if (base != null && base.getCode() == ResultCode.CODE_DEVICE_NOT_LOGIN) {
                    LoginCmd loginCmd = new LoginCmd(new CommandResultCallback() {
                        ...
                        public void onSuccess(String baseObtain) {
                            LogUtils.d(TAG, "设备重新登录成功.重新发送指令..");
                            try {
                                Response response = OkhttpClientManager.getInstance().getmOkHttpClient().newCall(request).execute();
                                if (response != null && response.body() != null) {
                                    String json = response.body().string();
                                    LogUtils.d(TAG, "<<--重试微聊:" + json);
                                    netCallBack.onSuccess(json);
                                } else {
                                    LogUtils.d(TAG, "<<--重试微聊 fail" );
                                    netCallBack.onFail();
                                }
                            } catch (IOException e) {
                                e.printStackTrace();
    ...
    public void getGroupListData(final List<WechatGroupInfo> localList, final NetCallBack<List<WechatGroupInfo>> callBack) {
        ...
        req(url, new NetCallBack<String>() {
                ...
                if (listGroupObtain != null && listGroupObtain.getCode() == 0 && listGroupObtain.getData() != null
                    ...
                    if (localList != null && localList.size() > 0) {
                        for (WechatGroupInfo w : localList) {
                            LogUtils.d(TAG, "本地数据条数,比服务器多" + w.toString());
                            w.delete();
                        }
                    }
                    if (callBack != null) {
                        callBack.onSuccess(wechatGroupInfoList);
                        LogUtils.d(TAG, "getGroupListData success" );
                    }
                } else {
                    //服务器没有数据了,删除本地群组
                    ...
                    if (callBack != null) {
                        callBack.onFail();
                        LogUtils.d(TAG, "getGroupListData fail" );
                    }
                }
            }
            ...
            public void onFail() {
                if (callBack != null) {
                    callBack.onFail();
                    LogUtils.d(TAG, "getGroupListData fail2" );
                }
            }
        });
    }
    ...
    private void uploadGroupVoice(String groupId, String filePath, NetCallBack<UploadGroupVoiceObtain> callBack) {
        if (!hasNet()) {
            if (callBack != null) {
                callBack.onFail();
                LogUtils.d(TAG, "uploadGroupVoice,no net");
            }
            return;
        }
        ...
        try {
            ...
            LogUtils.w(TAG, "上传群语音失败~" + e.toString());
            if (callBack != null) {
                callBack.onFail();
            }
        }
    ...
    private void sendGroupMsg(String groupId, int msgType, int msgLength, String msgContent, NetCallBack<SendGroupMsgObtain> callBack) {
        ...
        try {
            ...
            jsonObject.put("msgLength",msgLength);
            jsonObject.put("msgContent",msgContent);
            final MediaType JSON = MediaType.parse("application/json; charset=utf-8");
            RequestBody body = RequestBody.create(JSON,jsonObject.toString());
            LogUtils.d(TAG, "-->>发送群消息:" + jsonObject.toString());
            Request request = new Request.Builder()
            ...
            if (callBack != null) {
                callBack.onFail();
                LogUtils.w(TAG, "发送群消息失败~");
            }
        } catch (IOException | JSONException e) {
            LogUtils.w(TAG, "发送群消息失败~~e=" + e.toString());
            if (callBack != null) {
                callBack.onFail();
            }
        }
    ...
    public void getGroupMsg(String groupId, int msgId, int pageSize, final NetCallBack<List<WechatMessageInfo>> callBack) {
        String url = SocketConstant.getGroupMsg
                + "?imei="+Config.IMEI+"&groupId="+groupId+"&msgId="+msgId+"&pageSize="+pageSize;
        req(url, new NetCallBack<String>() {
            @Override
            public void onSuccess(String json) {
                ...
                if (obtain != null && obtain.getCode() == 0 && obtain.getData() != null) {
                    ...
                    if (callBack != null) {
                        ...
                        LogUtils.i(TAG,"获取群最新消息成功");
                    }
                } else {
                    if (callBack != null) {
                        callBack.onFail();
                        LogUtils.i(TAG,"获取群最新消息失败");
                    }
                }
            }
    ...
    public void listGroupMsg(String groupId, int msgId, int pageNum, int pageSize, final NetCallBack<List<WechatMessageInfo>> callBack) {
        ...
        String url = SocketConstant.listGroupMsg
                + "?imei=" +Config.IMEI+"&groupId="+groupId+"&msgId="+msgId+"&pageNum="+pageNum+"&pageSize="+pageSize;
        req(url, new NetCallBack<String>() {
            @Override
            public void onSuccess(String json) {
                List<WechatMessageInfo> list;
                ...
                if (obtain != null && obtain.getCode() == 0 && obtain.getData() != null) {
                    ...
                    if (callBack != null) {
                        callBack.onSuccess(list);
                        LogUtils.i(TAG,"获取群历史消息成功");
                    }
                } else {
                    if (callBack != null) {
                        callBack.onFail();
                        LogUtils.i(TAG,"获取群历史消息失败");
                    }
                }
            }
