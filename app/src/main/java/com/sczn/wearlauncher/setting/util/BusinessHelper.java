...
public class BusinessHelper {
    ...
    public static void requestAudioFocus(Context context) {
        ...
        if (resultMusic == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
            LogUtils.d(TAG, "resultMusic 申请到权限了！！");
        } else {
            LogUtils.d(TAG, "resultMusic 没有申请到权限了！！ result : " + resultMusic);
        }

        if (resultAlarm == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
            LogUtils.d(TAG, "resultAlarm 申请到权限了！！");
        } else {
            LogUtils.d(TAG, "resultAlarm 没有申请到权限了！！ result : " + resultAlarm);
        }
    }
}
