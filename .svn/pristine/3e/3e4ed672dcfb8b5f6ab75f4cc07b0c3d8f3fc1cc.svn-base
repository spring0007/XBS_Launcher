...
public class WifiAdminBiz {
    ...
    public void checkNetCardState() {
        if (mWifiManager.getWifiState() == 0) {
            LogUtils.i("tag_taoyunwatch", "网卡正在关闭");
        } else if (mWifiManager.getWifiState() == 1) {
            LogUtils.i("tag_taoyunwatch", "网卡已经关闭");
        } else if (mWifiManager.getWifiState() == 2) {
            LogUtils.i("tag_taoyunwatch", "网卡正在打开");
        } else if (mWifiManager.getWifiState() == 3) {
            LogUtils.i("tag_taoyunwatch", "网卡已经打开");
        } else {
            LogUtils.i("tag_taoyunwatch", "..没有获取到状态---_---");
        }
    }
    ...
    public int addNetwork(WifiConfiguration wcg) {
        ...
        LogUtils.i(TAG, "connect success? " + flag + "    wcgID =" + wcgID);
    ...
    public void connectWifi(String ssid, String pwd) {
        ...
        LogUtils.d(TAG, "wifiBiz.isConnected() =" + isConnected());
        LogUtils.d(TAG, "wifiBiz.lastSsidFromPush =" + lastSsidFromPush);
    ...
    private void onReceiveNewNetworks(List<ScanResult> mWifiList, String ssidFromPush, String pwdFromPush) {
        ...
        LogUtils.d(TAG, "============onReceiveNewNetworks============");

        if (ssidFromPush == null || pwdFromPush == null)
            return;
        for (ScanResult result : mWifiList) {
            LogUtils.d(TAG, "result.SSID : " + result.SSID);
        ...
        if (mWifiManager.getWifiState() != WifiManager.WIFI_STATE_ENABLED) {
            ...
            synchronized (this) {
                LogUtils.d(TAG, "============onReceiveNewNetworks============" + ssidFromPush + "/" + pwdFromPush);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (WifiManager.WIFI_STATE_CHANGED_ACTION.equals(action)) {
                ...
                if (WifiManager.WIFI_STATE_ENABLED == wifiState) {
                    synchronized (this) {
                        WifiConfiguration config = WifiConfigurationAdmin.createWifiWpaInfo(ssidFromPush, pwdFromPush);
                        LogUtils.d(TAG, "ssid = " + ssidFromPush + ",pwd = " + pwdFromPush);
                        addNetwork(config);
                    }
                    unregisterReceiver();
                }
