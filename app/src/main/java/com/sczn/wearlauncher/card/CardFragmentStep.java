...
public class CardFragmentStep extends absDialogFragment {
    ...
    private void freshView() {
        ThreadUtil.getPool().execute(new Runnable() {
            @Override
            public void run() {
                final int step = SystemPermissionUtil.getCurrentStepData(getActivity());
    ...
    private void updateStepUI(int step ){
        ...
        mDis.setText(UserInfoUtil.getDisByStep(getActivity(),step));
        mCal.setText(UserInfoUtil.getKcalByStep(getActivity(),step));
