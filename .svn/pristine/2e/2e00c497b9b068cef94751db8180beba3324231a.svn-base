...
public class ChatGroupListActivity extends AbsActivity implements ObserverListener {
    ...
    private void defaultData() {
      ...
      /*  WechatGroupInfo wechatGroupInfo = new WechatGroupInfo();
        ...
        //water:lqq
        NetHttp.getNet().getGroupListData(list, new NetCallBack<List<WechatGroupInfo>>() {
            @Override
            public void onSuccess(final List<WechatGroupInfo> wechatGroupInfos) {
                LogUtils.i(TAG,"同步群列表成功,wechatGroupInfos"+wechatGroupInfos.toString());
                if(groupListAdapter != null) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            groupListAdapter.addData(wechatGroupInfos);
                        }
                    });
                }
            }
