...
import com.sczn.wearlauncher.socket.command.CmdReceive;
...
public class WaterSocketService extends Service {
    ...
    private static final long HEART_BEAT_RATE_TIME =  180000;//定义3分钟秒检查一次
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what) {
                case HEART_BEAT_RATE:
                    ...
                    if (t > 60000) {
                        LogUtils.d(TAG, "设备1分钟内没有调用send方法,需要发送心跳包,维持连接");
                        mHandler.postDelayed(heartRunnable, 0);
                    } else {
                        LogUtils.d(TAG, "不需要维持:" + t);
                    }
                    mHandler.sendEmptyMessageDelayed(HEART_BEAT_RATE, HEART_BEAT_RATE_TIME);
                    break;
                ...
                if (mSocket != null) {
                    ...
                    while (!mSocket.isClosed() && !mSocket.isInputShutdown() && ((length = is.read(buffer)) != -1)) {
                        if (length > 0) {
                            ...
                            LogUtils.i(TAG,"receiverMessage="+receiverMessage); //返回数据
                            //water:lqq
    ...
    private void initSocket() {
        if (!NetworkUtils.isNetWorkConnected(LauncherApp.getAppContext())) {
            LogUtils.d(TAG, "init socket not network connected.");
            return;
        }
            ...
            if (connect) {
               ...
               /* if (isReConnectSocket) {
                    mHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            CmdSendUtils.getHelper().heartbeat();
                        }
                    }, 2000);
            ...
            LoginCmd loginCmd = new LoginCmd(new CommandResultCallback() {
                ...
                public void onSuccess(String baseObtain) {
                    ...
                    mHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                          CmdSendUtils.getHelper().heartbeat(1000);
                        }
                    }, 2000);
                }
    ...
    public boolean send(final BaseCommand command) {
        LogUtils.d(TAG, "send:" + command.send());
        if (command.send() == null || command.send().isEmpty()) {
            LogUtils.d(TAG, "command isEmpty...");
            return false;
        }
        if (!NetworkUtils.isNetWorkConnected(LauncherApp.getAppContext())) {
            LogUtils.d(TAG, "send command not network connected.");
            return false;
        }
        if (null == mSocket) {
            LogUtils.w(TAG, "mSocket==null");
            return false;
        }
            ...
            try {
                ...
                if (command.getCmd() != null && command.getCmd().equals(CmdCode.TK)) {
                    if (command.getData() != null && command.getData().length > 0) {
                        LogUtils.d(TAG, "" + CommandHelper.getInstance().hexString(command.getData()));
                ...
                e.printStackTrace();
                mSocket = null;
                LogUtils.e(TAG, "mSocket==null 发送失败");
                return false;
            }
        } else {
            LogUtils.i(TAG, "mSocket close ");
            return false;
        }
    }
