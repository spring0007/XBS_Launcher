...
import com.sczn.wearlauncher.socket.command.post.LocationCmd;
...
public class LocationCallBackHelper {
    ...
    public void startLocation(final Context mContext, OnLocationChangeListener<LocationInfo> onLocationChangeListener) {
        ...
        LogUtils.d(TAG, "开始定位,是否有网络:" + hasNet);
        if (!hasNet) {
            /**
        ...
        if (info == null || info.isEmpty() || current - lastTime >= 60000) {
            LogUtils.i(TAG, "定位信息为空或超时，重新获取信息");
            //读取基站信息
            doOnBaseInfo(mContext);
            //读取WiFi信息
            doOnWiFiInfo(mContext);
        } else {
            LogUtils.i(TAG, "两次请求定位的时间,小于1分钟,直接使用本地上次记录的位置信息:" + info);
            try {
                locationInfo = GsonHelper.getInstance().getGson().fromJson(info, LocationInfo.class);
                //判断电子围栏是否越界
                FenceHelper.checkFence(locationInfo.getLng(),locationInfo.getLat());
            ...
            public void onCallback() {
                startLocation(mContext, new OnLocationChangeListener<LocationInfo>() {
                    ...
                    public void success(LocationInfo info) {
                        LogUtils.i(TAG,"获取定位信息成功，并上传");
                        doSendLocationCmd(info);
                    }

                    @Override
                    public void fail() {
                        LogUtils.i(TAG,"获取定位信息失败");
                    }
                });
            }
        });
     ...
     * @param mContext
     */
    public void startShutDownAndUpload(final Context mContext) {
        MoreFastEvent.getInstance().event(7000, new MoreFastEvent.onEventCallBackListener() {
            @Override
            public void onCallback() {
                startLocation(mContext, new OnLocationChangeListener<LocationInfo>() {
                    @Override
                    public void success(LocationInfo info) {
                        LogUtils.i(TAG,"关机获取定位信息成功，并上传");
                        doSendShutDownCmd(info,mContext);
                    }

                    @Override
                    public void fail() {
                        LogUtils.i(TAG,"关机获取定位信息失败");
                    }
                });
            }
        });
     ...
     * @param info
     */
    public void doSendShutDownCmd(final LocationInfo info, final Context mContext) {
        //定位成功,记录定位的时间和信息
        ...
        WaterSocketManager.getInstance().send(new ShutDownCmd(info, new CommandResultCallback() {
            @Override
            public void onSuccess(String baseObtain) {
                LogUtils.i(TAG, "doSendShutDownCmd,关机定位成功.");
                SystemPermissionUtil.shutdown(mContext);

            }

            @Override
            public void onFail() {
                LogUtils.w(TAG, "doSendShutDownCmd,关机定位失败.");
                SystemPermissionUtil.shutdown(mContext);
            }
        }));
    }
    ...
    private void doOnWiFiInfo(final Context mContext) {
        WiFiLocationHelper.getInstance().scanWifiData(mContext, new OnLocationChangeListener<List<WifiDataBean>>() {
            ...
            public void success(List<WifiDataBean> data) {
                ...
                LogUtils.d(TAG, "读取到有WiFi信息..准备上传," + info);
                ...
                if (data.size() > 3) {
                    ...
                    LogUtils.d(TAG, "判断周围的WiFi数量小于3,使用GPS定位");
                    doOnGPS();
                }
            }
            ...
            @Override
            public void fail() {
                LogUtils.i(TAG, "读取失败or没有WiFi信息..开始GPS定位");
                doOnGPS();
            }
        });
    }
    ...
    private void doOnStartGPSLocation() {
        ...
        GPSHelper.getInstance().startGPS(LauncherApp.getAppContext(), new OnLocationChangeListener<LocationInfo>() {
            ...
            public void fail() {
                stopLocation();
                LogUtils.i(TAG, "GPS定位失败了,开始上传基站定位");
                locationInfo.setType(0);
                if (onLocationChangeListener != null) {
                    onLocationChangeListener.success(locationInfo);
                    onLocationChangeListener = null;
     ...
     * @param info
     *//*
    public void doSendLocationUDCmd(final LocationInfo info) {
        //定位成功,记录定位的时间和信息
        SPUtils.setParam(SPKey.LOCATION_LAST_TIME, System.currentTimeMillis());
        SPUtils.setParam(SPKey.LOCATION_LAST_INFO, GsonHelper.getInstance().getGson().toJson(info));
        SPUtils.setParam(SPKey.LOCATION_LAST_UPLOAD_TIME, System.currentTimeMillis());
        WaterSocketManager.getInstance().send(new UD(info, new CommandResultCallback() {
            @Override
            public void onSuccess(String baseObtain) {
                MxyLog.i(TAG, "doSendLocationUDCmd,上传定位成功.");
                locationUploadSuccessful(baseObtain);
            }

            @Override
            public void onFail() {
                MxyLog.w(TAG, "doSendLocationUDCmd,上传定位失败.");
                locationUploadFail();
            }
        }));
    }*/
    ...
    public void doSendLocationCmd(final LocationInfo info) {
        ...
        SPUtils.setParam(SPKey.LOCATION_LAST_INFO, GsonHelper.getInstance().getGson().toJson(info));
        SPUtils.setParam(SPKey.LOCATION_LAST_UPLOAD_TIME, System.currentTimeMillis());
        WaterSocketManager.getInstance().send(new LocationCmd(info, new CommandResultCallback() {
            @Override
            public void onSuccess(String baseObtain) {
                LogUtils.i(TAG, "doSendLocationCmd,上传定位成功.");
                locationUploadSuccessful(baseObtain);
            }

            @Override
            public void onFail() {
                LogUtils.w(TAG, "doSendLocationCmd,上传定位失败.");
                locationUploadFail();
            }
        }));
    }
        ...
        public boolean handleMessage(Message message) {
            switch (message.what) {
                case START_LOAD_LOCATION_END:
                    LogUtils.d(TAG, "GPS定位超时了,开始上传基站定位");
                    stopLocation();
                    locationInfo.setType(0);
                    if (onLocationChangeListener != null) {
                        onLocationChangeListener.success(locationInfo);
