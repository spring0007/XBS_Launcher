...
public class CmdGprsReceive {
    ...
    public static void onResult(String result, byte[] buffer) {
        LogUtils.i("parseData", "接收的数据:" + result);
        if (!StringUtils.isEmpty(result)) {
            if (!result.contains("*")) {
                WaterSocketManager.getInstance().onFail();
                LogUtils.d(TAG, "收到错误数据.数据中没有包含协议必要的*");
                return;
            }
            //去掉头尾的[]符号
            ...
            LogUtils.i(TAG, "收到的类型<<<<< = " + flag);
            switch (flag) {
                case CmdCode.LK:
                    break;
                case CmdCode.VERNO:
                    LogUtils.i(TAG, "当前版本号:" + Build.DISPLAY);
