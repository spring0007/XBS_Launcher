...
public class SocketService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        LogUtils.i(TAG, "wakeupTimes:" + wakeupTimes + ",isNetworkAvailable:" + NetworkUtils.isNetworkAvailable(SocketService.this));
    ...
    public boolean sendMsg(String serverSquence, String msg) {
        ...
        LogUtils.d("----------send to server-------", msg1);
        return true;
    }
                ...
                try {
                    ...
                    while (!socket.isClosed() && !socket.isInputShutdown()
                        ...
                        if (length > 0) {
                            ...
                            LogUtils.d("----------get from server-------", receiverMessage);
    ...
    private void dataDeal(String msg) {
        ...
        LogUtils.i("---dataDeal---", "1001" + msg.substring(8, 12) + " " + msg);
        if (flag.equals("1001")) {//登录包响应

            LogUtils.i("---dataDeal---", "1001" + msg.substring(msg.length() - 6, msg.length() - 4));
            mHandler.sendEmptyMessageDelayed(8, 10000);
        } else if (flag.equals("1002")) {//心跳包响应


            LogUtils.i("---dataDeal---", "1002" + msg.substring(msg.length() - 6, msg.length() - 4));
            ...
            for (int i = 1; i <= sum; i++) {
                ...
                LogUtils.i("----------111111-------", Settings.Global.getString(getApplication().getContentResolver(), "isCall") +
            ...
            if (doWhat == 1) {
                ...
                try {
                    ...
                    LogUtils.i(TAG, "----------serverSquence-------" + peopleImage);
    ...
    private void acquireWakeLock() {
        LogUtils.d(TAG, "Acquiring wake lock");
        if (mWakeLock == null) {
            ...
            if (NetworkUtils.isNetworkAvailable(SocketService.this)) {
                if (System.currentTimeMillis() - serverRespondTime >= 5 * 60 * 1000) {
                    ...
                    ThreadUtil.getPool().execute(new Runnable() {
                        @Override
                        public void run() {
                            LogUtils.d(TAG, "get server socket address");
                            try {
