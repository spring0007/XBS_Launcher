...
import com.sczn.wearlauncher.socket.command.post.ShakeCmd;
...
public class ShakeActivity extends AbsActivity {
    ...
    private static final int END_ANIM = 0x4;
                ...
                Thread thread = new Thread() {
                    ...
                    public void run() {
                        super.run();
                            ...
                            //再来一次震动提示
                            mHandler.sendEmptyMessageDelayed(AGAIN_SHAKE,500);
                            mHandler.sendEmptyMessageDelayed(END_SHAKE,1000);
    ...
    private void doOnSearchFriends() {
        mHandler.sendEmptyMessageDelayed(END_ANIM,5000);
        LocationCallBackHelper.getInstance().startBS(LauncherApp.getAppContext(), new OnLocationChangeListener<LocationInfo>() {
            ...
            public void success(LocationInfo info) {
                ShakeCmd pp = new ShakeCmd(info.getLat(),info.getLng(), new CommandResultCallback() {
                    @Override
                    public void onSuccess(String baseObtain) {
                        endShake(true);
                        LogUtils.i(TAG,"摇一摇 success");
                    }

                    @Override
                    public void onFail() {
                        endShake(false);
                        LogUtils.i(TAG,"摇一摇 fail");
                    }
                });
    ...
    protected void onStart() {
        ...
        if (mSensorManager != null) {
            ...
            if (mAccelerometerSensor != null) {
                LogUtils.i(TAG,"mAccelerometerSensor");
                mSensorManager.registerListener(myAccelerometerListener, mAccelerometerSensor, SensorManager.SENSOR_DELAY_NORMAL);
            }
        }
    }
    ...
    protected void onPause() {
        ...
        endShake(false);
        mHandler.removeMessages(END_SHAKE);
        mHandler.removeMessages(START_SHAKE);
        mHandler.removeMessages(AGAIN_SHAKE);
        mHandler.removeMessages(END_ANIM);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case END_SHAKE:
                    ...
                    break;
                case END_ANIM://延迟5秒后，没有收到交朋友信息结束
                    endShake(false);
                    break;
            }
        }
