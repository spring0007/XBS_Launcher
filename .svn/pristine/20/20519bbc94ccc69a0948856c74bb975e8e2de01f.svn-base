...
public class WaterSocketManager {
    ...
    private void sendOrWait(BaseCommand command, boolean needCallBack) {
        ...
        if (socketService == null) {
            ...
            LogUtils.d(TAG, "send..socketService==null");
        }
        while (true) {
            LogUtils.d(TAG, "send msg ");
            if (socketService == null) {
                LogUtils.d(TAG, "error socketService == null");
                mIsBindService = false;
                registerService();
                onFail();
                break;
            ...
            if (!NetworkUtils.isNetworkAvailable(LauncherApp.getAppContext())) {
                LogUtils.d(TAG, "error isNetworkAvailable false");
                onFail();
                break;
            }
            //如果需要增加队列等待发送的话,在这里检查commandBlockingDeque的size,如果有的大于0的话,需要进行等待操作
            if (commandBlockingDeque != null && commandBlockingDeque.size() > 0) {
                LogUtils.d(TAG, "has cmd send need waiting..");
                waitTime += CHECK_TIME;
                ThreadUtil.threadDelayTime(CHECK_TIME);
                if (waitTime > 10 * CHECK_TIME) {
                    LogUtils.d(TAG, "waiting command send error");
                    onFail();
                    break;
                }
            } else {
                if (socketService.getmSocket() == null || !socketService.getmSocket().isConnected()) {
                    ...
                    if (reconnectCounts < 2) {
                        LogUtils.d(TAG, "socket == null or socket not connected try to reconnect socket");
                        reconnectCounts++;
                        reInitSocket();
                        ThreadUtil.threadDelayTime(10000);
                    } else {
                        LogUtils.d(TAG, "socket == null or socket not connected reconnect socket error");
                        onFail();
                        break;
                    }
                } else {
                    ...
                    if (result) {
                        LogUtils.d(TAG, "send msg success.wait callback");
                        ...
                        if (needCallBack) {
                            ...
                            LogUtils.d(TAG, "send msg do not need call back.");
                        }
                        break;
                    } else {
                        if (socketService != null && socketService.getmSocket() != null) {
                            ...
                            if (connected) {
                                LogUtils.d(TAG, command + "..wait send...");
                                ThreadUtil.threadDelayTime(CHECK_TIME);
                                if (waitTime > 10 * CHECK_TIME) {
                                    LogUtils.d(TAG, "command send error");
                                    onFail();
                                    break;
                                } else {
                                    LogUtils.d(TAG, "retry command send");
                                }
                            } else {
                                //需要重新初始化socket
                                if (reconnectCounts < 2) {
                                    LogUtils.d(TAG, "try to reconnect socket");
                                    reconnectCounts++;
                                    reInitSocket();
                                    ThreadUtil.threadDelayTime(10000);
                                } else {
                                    LogUtils.d(TAG, "reconnect socket error");
                                    onFail();
                                    break;
                                }
                            }
    ...
    private Handler mHandler = new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            if (msg.what == WAIT_MSG_CALLBACK_TIME_OUT) {
                LogUtils.d(TAG, "指令发送成功,到了设定的时间,接收指令超时..");
                onFail();
                //超时之后,重新初始化socket
                reInitSocket();
            }
