...
public class SystemPermissionUtil {
    ...
    public static void shutdown(Context mContext) {
        ...
        new TcCmccWatchDevice().shutdown(mContext,false);
    }
    ...
    public static void sendSMS(String phoneNumber, String message) {
        if (!NetworkUtils.isHasSimCard(LauncherApp.getAppContext())) {
            LogUtils.e(LogUtils.TAG, "no has sim card");
            return;
        }
        // 获取短信管理器
    ...
    public static void doCalling(String number) {
        LogUtils.d("doCalling", "call to number.." + number);
    ...
    public static void oneWayDoCalling(String number) {
        LogUtils.d("oneWayDoCalling", "call to number.." + number);
    ...
    public static void getBtyState() {
        ...
        LogUtils.d(LogUtils.TAG, "isCharging:" + isCharging);
        LogUtils.d(LogUtils.TAG, "status:" + status);
        LogUtils.d(LogUtils.TAG, "percent:" + percent);
     ...
     * @return
     */
    public static int getCurrentStepData(Context mContext) {
        String stepStr = new TcCmccWatchDevice().getStepDataByNow(mContext);
        return Integer.valueOf(stepStr);
    }
     ...
     */
    public static void setIndividualListening(Context context,JSONObject jsonObject) throws JSONException {
        JSONObject object = jsonObject.getJSONObject("data");
        if(object!=null) {
            String stepNumber = object.getString("optUserPhone");
            new TcCmccWatchDevice().enableVoiceMonitor(context,true);
            Intent intent = new Intent(Intent.ACTION_CALL);
            Uri data = Uri.parse("tel:" + stepNumber);
            intent.setData(data);
            context.startActivity(intent);
            LogUtils.i("SystemPermissionUtil","setIndividualListening");
        }
    }
}
