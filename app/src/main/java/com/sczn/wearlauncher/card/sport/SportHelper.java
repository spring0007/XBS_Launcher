...
public class SportHelper {
     ...
     */
    public void uploadStep( String  optUserId) throws NumberFormatException {
        int  step = SystemPermissionUtil.getCurrentStepData(LauncherApp.getAppContext());
        //上传数据
        UploadStepsCmd stepsCmd = new UploadStepsCmd(step, TimeUtil.getCurrentTimeSec(), optUserId, new CommandResultCallback() {
            ...
            public void onSuccess(String baseObtain) {
                LogUtils.d(TAG, "上传运动数据成功.");
            }

            @Override
            public void onFail() {
                LogUtils.d(TAG, "上传运动数据失败.");
            }
        });
     ...
     * @return
     */
    public int getCurrentStepDatabyWeek(Context mContext) {
        int step = 0;
