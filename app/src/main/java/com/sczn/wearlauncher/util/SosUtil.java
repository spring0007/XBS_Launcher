...
public class SosUtil {
     ...
     */
    private void reCall() {
        LogUtils.i(TAG, "reCall callCount:" + callCount);
    ...
    private class PhoneStatusReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action != null && action.equals(ACTION_CALL_CONNECTED)) {
                LogUtils.w(TAG, "CallConnectedStateBroadCast Receiver"); //电话接通
                dealConnected();
            } else if (action != null && action.equals(ACTION_CALL_HANGUP)) {
                LogUtils.w(TAG, "CallHANGUPStateBroadCast Receiver"); //电话挂断
                dealHangup(intent);
            }
        }

        private void dealConnected() {
            LogUtils.w(TAG, "CallConnectedStateBroadCast Receiver isCallerOrCallee=" + isCallerOrCallee); //电话接通
        }
        ...
        private void dealHangup(Intent intent) {
            ...
            if (!isRingingOrConnected) {
                LogUtils.i(TAG, "sos接通..sos重拨流程结束");
                if (onSosCallBack != null) {
            ...
            if (!isCallerOrCalleeHangUp && isRingingOrConnected) {
                LogUtils.i(TAG, "sos执行重拨...");
                reCall();
            } else {
                LogUtils.i(TAG, "接通或者主动挂断...");
