...
public class ChartView extends View {
    ...
    protected void init(Context context, AttributeSet attrs){
        ...
        mStartColor = array.getColor(R.styleable.ChartView_pathStartColor,Color.BLACK);
        ...
        int textColor = array.getColor(R.styleable.ChartView_textColor, Color.BLACK);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0;i<leftStr.length;i++){
            //鐢诲乏杈圭殑瀛�
            canvas.drawText(leftStr[i], bottomGap / 2 + padding - (textPaint.measureText(leftStr[i]) / 2), getHeight() - i * leftGap - fontHeight / 6 * 5, textPaint);
            //鐢绘í绾�
