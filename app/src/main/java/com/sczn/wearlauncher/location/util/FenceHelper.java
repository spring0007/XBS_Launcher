...
public class FenceHelper {
    ...
    public static List<String> fenceRemindTimeList;
     /*{"a":1,"data":[{ "createTime": "2019-02-27 17:38:24","fenceLat":"22.256451", 纬度"fenceLng":"113.236547",经度"fenceRange": 200, 	安全范围 单位米
                "id": 2,"isEnable": true,	是否启用 "name": "学校",	围栏名称 "remindDays": 1,2,4, 	提醒周期   表示周一周二周四
                "remindTime": "12:35",	 提醒时间 "remindWay": 1, 	提醒方式(1-进入提醒，2-离开提醒，3-定时未进入提醒)
                "updateTime": "2019-02-27 17:38:24","watchId": 3},],
            "message":"操作成功！","no":null,"status":0,"timestamp":1551930285873,"type":14}*/
    ...
    public static void resolveJson(String json) {
        ...
        LogUtils.d(TAG, "共有" + fence.size() + "个电子围栏");
    ...
    public static void checkFence(double lng, double lat) {
        LogUtils.i(TAG, "checkFence:lng=" + lng + ",lat=" + lat);
        ...
        if (fences != null && fences.size() > 0) {
            ...
            for (FenceObtain f : fences) {
                ...
                if (f.getRemindWay() == 1) {
                    if (distance <= fRange) {
                        LogUtils.d(TAG, "提示,当前已进入" + f.getName() + "围栏.");
                    ...
                    if (distance > fRange) {
                        LogUtils.d(TAG, "当前的位置,超出了" + f.getName() + "围栏.");
                        if (f.getIsIsEnable() && isRemindDate(f.getRemindDays())) {
                            fenceAlarm(f.getName(), f.getFenceLng(), f.getFenceLat());
                        }
                    } else {
                    ...
                    LogUtils.d(TAG, "在哪个时间段,没有进入该区域." + f.getRemindTime());
                    if (distance > fRange) {
                        if (f.getIsIsEnable() && isRemindDate(f.getRemindDays())) {
                            //需要进行提醒
                        ...
                        LogUtils.d(TAG, "进入该正常范围区域.");
                    }
                }
            }
        } else {
            LogUtils.d(TAG, "暂时没有任务围栏信息.");
        }
    }
    ...
    private static void fenceAlarm(String fenceName, double lng, double lat) {
        FenceAlarmCmd alarmCmd = new FenceAlarmCmd(fenceName, lng, lat, new CommandResultCallback() {
            ...
            public void onSuccess(String baseObtain) {
                LogUtils.d(TAG, "上传电子围栏警告onSuccess");
            }

            @Override
            public void onFail() {
                LogUtils.d(TAG, "上传电子围栏警告onFail");
            }
        });
        ...
        LocationCallBackHelper.getInstance().startBS(LauncherApp.getAppContext(), new OnLocationChangeListener<LocationInfo>() {
            ...
            public void success(LocationInfo info) {
                AL al = new AL(info, status, new CommandResultCallback() {
                    @Override
                    public void onSuccess(String baseObtain) {
                        LogUtils.d(TAG, "上传围栏状态.onSuccess");
                    }

                    @Override
                    public void onFail() {
                        LogUtils.d(TAG, "上传围栏状态.onFail");
                    }
                });
            ...
            @Override
            public void fail() {
                LogUtils.d(TAG, "上传围栏状态.获取位置信息失败.");
            }
        });
    }
}
