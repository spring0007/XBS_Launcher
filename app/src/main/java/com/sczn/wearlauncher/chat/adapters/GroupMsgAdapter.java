...
public class GroupMsgAdapter extends RecyclerView.Adapter<GroupMsgAdapter.MyViewHolder> {
    ...
    private static final String TAG = "GroupMsgAdapter";
    ...
    public List<WechatMessageInfo> getDatas(){
        return this.mDatas;
    }
    ...
    public void addAllNotClear(List<WechatMessageInfo> list) {
        LogUtils.i(TAG,"addAllNotClear");
        if (list != null && list.size() > 0) {
            for (WechatMessageInfo info : list) {
                if (!mDatas.contains(info)) {
                    mDatas.add(info);
                    LogUtils.i(TAG,"addAllNotClear,info="+info.toString());
                }
            }
            notifyDataSetChanged();
        }
