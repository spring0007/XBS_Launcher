...
public class BaseStationHelper {
    ...
    public static BaseStationInfo getConnectedCellInfo(Context mContext) {
        ...
        LogUtils.i(TAG, "获取的基站信息是" + operator);
        if (operator == null || operator.length() < 5) {
            LogUtils.i(TAG, "获取基站信息有问题,可能是手机没插sim卡");
            return null;
        }
        ...
        if (cellLocation == null) {
            LogUtils.i(TAG, "手机没插sim卡吧");
            return null;
        }
        LogUtils.i(TAG, "通信运营商: " + getCarrier(operator));
        if (mTelephonyManager.getPhoneType() == TelephonyManager.PHONE_TYPE_GSM) {// 中国移动和中国联通获取LAC,CID的方式
            LogUtils.i(TAG, "当前连接的是gsm基站");
            ...
            LogUtils.e(TAG, "基站信息sid=" + cdma.getSystemId() + ",nid=" + cdma.getNetworkId() + ",bid=" + cdma.getBaseStationId());
            LogUtils.i(TAG, "--------------------------------------------------------");
            return baseStationInfo;
        } else {
            LogUtils.i(TAG, "---------------------现在不知道是什么基站-------------------------");
            return null;
        }
    }
    ...
    public static List<BaseStationInfo> getCellInfo(Context mContext) {
        ...
        if (infos != null) {
            if (infos.size() == 0) {
                LogUtils.i(TAG, "附近没有其他基站");
                list.size();
                return list;
            }
            LogUtils.i(TAG, "附近发现" + infos.size() + "个基站");
            for (CellInfo i : infos) { // 根据邻区总数进行循环
                LogUtils.d(TAG, "附近基站信息是" + i.toString());//这里如果出现很多cid
